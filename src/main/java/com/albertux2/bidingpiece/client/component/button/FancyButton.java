...
public class FancyButton extends Button {
    ...
    public void renderButton(MatrixStack ms, int mx, int my, float pt) {
        ...
        // Si el botón no está activo, reducimos la opacidad y usamos colores más apagados
        if (!this.active) {
            a *= 0.5f;
        }
        ...
        // Color del texto: normal, hover o desactivado
        int c = !this.active ? 0xFF808080 : this.isHovered() ? 0xFFFFFFFF : 0xFFEAEAEA;

        // Si está desactivado, dibujamos una versión "sombreada" del texto
        if (!this.active) {
